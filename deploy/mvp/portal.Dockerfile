...
ENV PYTHONDONTWRITEBYTECODE=1 \
    ...
    DEFENSE_TRACKER_BUILD_COMMIT=${GIT_SHA} \
    PORT=8080
...
COPY --chown=defense:defense feishu_webhook_security.py /app/feishu_webhook_security.py
COPY --chown=defense:defense product_version.py /app/product_version.py
COPY --chown=defense:defense version.json /app/version.json
