...
记录日期：2026-07-25（旧版未签名本地产物）

> 本页保留的是提交 `88d507f` 与旧 EXE 的历史验证证据，不代表当前
> `v9.0.0` 已正式发布，也不是下载页。正式 Windows 版本只能以 GitHub
> Immutable Release 中通过可信 Authenticode 签名、时间戳和校验和复验的资产为准；
> 当前状态以仓库根目录 `README.md` 的 Release status 为准。
