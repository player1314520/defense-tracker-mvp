-- Preserve the outcome of an accepted push when the client retries the same
-- event.  The current head is returned separately so clients can distinguish
-- "this event was applied" from "this version is still the current head".
begin;

-- Existing events cannot be assigned trustworthy request hashes after the
-- fact.  This rollout is intentionally limited to the empty Staging database
-- and clean Production installs.
lock table public.sync_events in access exclusive mode;
do $migration$
begin
    if exists (
        select 1 from public.sync_events
    ) then
        raise exception 'request hash migration requires empty sync_events';
    end if;
end;
$migration$;
...
alter table public.sync_events
    drop constraint if exists sync_events_request_hash_check;
alter table public.sync_events
    add constraint sync_events_request_hash_check check (
