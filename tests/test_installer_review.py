...
APP_REVIEWER_KEY_ID = "application-reviewer-2026"
INSTALLER_REVIEWER_KEY_ID = "installer-reviewer-2026"
...
def _write_registry(
    ...
    *,
    status: str = "active",
    reviewer_key_id: str = INSTALLER_REVIEWER_KEY_ID,
    scope: str = INSTALLER_REVIEW_SCOPE,
) -> None:
...
def _write_approval(
    ...
    *,
    ...
    evidence: dict[str, object] = {
        ...
        "reviewed_at_utc": "2026-08-28T01:02:03Z",
    }
    if extra:
        evidence.update(extra)
    write_canonical_json(evidence_path, evidence)
    signature_path.write_text(
        base64.b64encode(private_key.sign(evidence_path.read_bytes())).decode("ascii")
        + "\n",
        encoding="ascii",
    )
...
def _case(tmp_path: Path) -> dict[str, object]:
    ...
    registry = tmp_path / "installer-reviewers.json"
    _write_registry(registry, private_key)
    evidence = tmp_path / "installer-review-evidence.json"
    signature = tmp_path / "installer-review-evidence.sig"
    evidence_sha256 = _write_approval(
        request, evidence, signature, private_key
    )
    verify_args = {
        ...
        "application_reviewer_key_id": APP_REVIEWER_KEY_ID,
        "unsigned_installer": unsigned_installer,
        "extracted_payload_root": payload,
        "signed_application_inventory": signed_application_inventory,
        "iss_path": iss,
...
@pytest.mark.parametrize("missing", ["evidence", "signature"])
def test_missing_approval_material_fails_closed(tmp_path, missing):
    case = _case(tmp_path)
    args = dict(case["verify_args"])
    args[f"{missing}_path"] = tmp_path / f"missing-{missing}"
    with pytest.raises(ValueError, match="must be a regular file"):
        verify_installer_before_sign(**args)


def test_wrong_signature_is_rejected(tmp_path):
    case = _case(tmp_path)
    wrong_key = Ed25519PrivateKey.generate()
    case["signature"].write_text(
        base64.b64encode(wrong_key.sign(case["evidence"].read_bytes())).decode("ascii"),
        encoding="ascii",
    )
...
def test_inactive_registry_is_rejected(tmp_path):
    case = _case(tmp_path)
    _write_registry(case["registry"], case["private_key"], status="inactive")
    with pytest.raises(ValueError, match="registry is inactive"):
        verify_installer_before_sign(**case["verify_args"])


def test_same_application_and_installer_reviewer_key_is_rejected(tmp_path):
    case = _case(tmp_path)
    args = dict(case["verify_args"])
    args["application_reviewer_key_id"] = INSTALLER_REVIEWER_KEY_ID
    with pytest.raises(ValueError, match="must use different key IDs"):
        verify_installer_before_sign(**args)


def test_wrong_reviewer_scope_is_rejected(tmp_path):
    case = _case(tmp_path)
    _write_registry(case["registry"], case["private_key"], scope="application-review")
    with pytest.raises(ValueError, match="wrong scope"):
        verify_installer_before_sign(**case["verify_args"])


def test_exact_evidence_schema_rejects_resigned_extra_field(tmp_path):
    case = _case(tmp_path)
    new_sha = _write_approval(
        case["request"],
        case["evidence"],
        case["signature"],
        case["private_key"],
    ...
    args = dict(case["verify_args"])
    args["expected_evidence_sha256"] = new_sha
...
def test_repository_installer_reviewer_registry_is_inactive_and_empty():
    root = Path(__file__).resolve().parents[1]
    registry = json.loads(
        (root / "release" / "installer-reviewers.json").read_text(encoding="utf-8")
    )
    assert registry == {
        "schema": 1,
        "status": "inactive",
